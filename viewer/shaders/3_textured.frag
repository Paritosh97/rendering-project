...
vec4 CalcBlinnPhongSpecular(float F, vec4 halfwayDir)
{    
    float spec = pow(max(dot(vertNormal, halfwayDir), 0.0), shininess);

    return (F * spec * texture(colorTexture, textCoords));
}
...
{   
    // ambient shading
    vec4 ambient  = dirLight.ambient  * texture(colorTexture, textCoords);

    // diffuse shading
    float diff = max(dot(vertNormal, lightVector), 0.0);
    vec4 diffuse  = dirLight.diffuse  * diff * texture(colorTexture, textCoords);
    ...
    {
        case 0:
            specular = lightIntensity * CalcBlinnPhongSpecular(F, halfwayDir) * texture(colorTexture, textCoords);
            break;
        case 1:
            specular = lightIntensity * CalcCookTorranceSpecular(F, halfwayDir) * texture(colorTexture, textCoords);
            break;
        case 2:
